//! Foundation NSXPC transport for the desktop structural-diff host.
//!
//! ```text
//!  Studio (Node)                 launchd service
//!  ────────────                  ──────────────
//!  harness-diff-client <NSXPC> harness-diff-xpc <stdio> harness-diff-host
//! ```
//!
...
const SERVICE: &str = "com.qoder.harness-studio.diff";
const DRIVER_BIN: &str = "harness-diff-host";
const REAP_GRACE: Duration = Duration::from_secs(5);

unsafe extern "C" {
    fn harness_diff_host_protocol() -> *const AnyProtocol;
    fn harness_diff_client_protocol() -> *const AnyProtocol;
}

fn host_interface() -> Retained<NSXPCInterface> {
    unsafe { NSXPCInterface::interfaceWithProtocol(&*harness_diff_host_protocol()) }
}

fn client_interface() -> Retained<NSXPCInterface> {
    unsafe { NSXPCInterface::interfaceWithProtocol(&*harness_diff_client_protocol()) }
}
...
define_class!(
    ...
    #[name = "HarnessDiffSession"]
    #[ivars = SharedDriver]
    struct DiffSession;

    unsafe impl NSObjectProtocol for DiffSession {}
...
define_class!(
    #[unsafe(super = NSObject)]
    #[name = "HarnessDiffListenerDelegate"]
    ...
    unsafe impl NSXPCListenerDelegate for ListenerDelegate {
        ...
        fn accept(&self, _listener: &NSXPCListener, connection: &NSXPCConnection) -> bool {
            match accept_connection(connection) {
                Ok(()) => true,
                Err(error) => {
                    eprintln!("[diff-host] refused a connection: {error}");
                    false
                }
            }
        }
...
fn accept_connection(connection: &NSXPCConnection) -> io::Result<()> {
    let driver_path = std::env::current_exe()?
        ...
        .ok_or_else(|| io::Error::other("cannot locate the harness-diff-host driver"))?;
    ...
    let session = DiffSession::new(shared.clone());
...
fn pump_driver_stdout(
    ...
    let reason = loop {
        ...
        let read = reader
            ...
            .read_until(b'\n', &mut line);
        match read {
            Ok(0) => break "the diff driver exited",
            Ok(_) if line.len() > MAX_FRAME_BYTES => {
                break "the diff driver emitted an oversized frame";
            }
            Ok(_) => deliver_frame(&proxy, &line),
            Err(_) => break "reading the diff driver's output failed",
        }
    };
...
define_class!(
    #[unsafe(super = NSObject)]
    #[name = "HarnessDiffBridge"]
    struct DiffBridge;

    unsafe impl NSObjectProtocol for DiffBridge {}

    impl DiffBridge {
        #[unsafe(method(deliverFrame:))]
        fn deliver_frame(&self, frame: &NSData) {
            if frame.len() > MAX_FRAME_BYTES {
                bridge_die("the diff service delivered an oversized frame");
            }
            let bytes = frame.to_vec();
...
pub fn bridge() -> std::process::ExitCode {
    autoreleasepool(|_| {
        ...
        let exported = DiffBridge::new();
        unsafe { connection.setExportedObject(Some(&exported)) };

        let interrupted = RcBlock::new(|| bridge_die("the diff service was interrupted"));
        connection.setInterruptionHandler(Some(&interrupted));
        let invalidated =
            RcBlock::new(|| bridge_die("the diff service connection was invalidated"));
