//! Foundation NSXPC transport for the desktop pty host.
//!
//! ```text
//!  Studio (Node)                 launchd service
//!  ────────────                  ──────────────
//!  harness-pty-client <NSXPC> harness-pty-xpc <stdio> harness-pty-host
//! ```
//!
//! No terminal logic lives here. This is the same transport shell as the arch
//! and diff hosts — one unmodified `harness-pty-host` driver per connection, and
//! frames tunnelled in both directions. It works unchanged for a duplex host
//! because it never inspects a frame: replies and `pty.data`/`pty.exit` events
//! both travel the driver's stdout to `deliverFrame`, and the caller's requests
//! travel `sendFrame` to the driver's stdin.
...
const SERVICE: &str = "com.qoder.harness-studio.pty";
const DRIVER_BIN: &str = "harness-pty-host";
const REAP_GRACE: Duration = Duration::from_secs(5);

unsafe extern "C" {
    fn harness_pty_host_protocol() -> *const AnyProtocol;
    fn harness_pty_client_protocol() -> *const AnyProtocol;
}

fn host_interface() -> Retained<NSXPCInterface> {
    unsafe { NSXPCInterface::interfaceWithProtocol(&*harness_pty_host_protocol()) }
}

fn client_interface() -> Retained<NSXPCInterface> {
    unsafe { NSXPCInterface::interfaceWithProtocol(&*harness_pty_client_protocol()) }
}
...
fn reap_driver(shared: &SharedDriver) {
    ...
    std::thread::spawn(move || {
        // Dropping stdin lets the driver hang up its own sessions (SIGHUP on the
        // masters) and exit, so the reap is orderly rather than an immediate kill.
        drop(owned.stdin);
...
define_class!(
    ...
    #[name = "HarnessPtySession"]
    #[ivars = SharedDriver]
    struct PtySession;

    unsafe impl NSObjectProtocol for PtySession {}
    ...
    impl PtySession {
        ...
        fn send_frame(&self, frame: &NSData) {
            ...
            // the driver ignores blank lines, so it is forwarded harmlessly.
            let mut guard = self
                .ivars()
                .lock()
                .unwrap_or_else(|poison| poison.into_inner());
...
define_class!(
    #[unsafe(super = NSObject)]
    #[name = "HarnessPtyListenerDelegate"]
    ...
    unsafe impl NSXPCListenerDelegate for ListenerDelegate {
        ...
        fn accept(&self, _listener: &NSXPCListener, connection: &NSXPCConnection) -> bool {
            match accept_connection(connection) {
                Ok(()) => true,
                Err(error) => {
                    eprintln!("[pty-host] refused a connection: {error}");
                    false
                }
            }
        }
...
fn accept_connection(connection: &NSXPCConnection) -> io::Result<()> {
    let driver_path = std::env::current_exe()?
        ...
        .ok_or_else(|| io::Error::other("cannot locate the pty host driver"))?;
    ...
    let session = PtySession::new(shared.clone());
...
fn pump_driver_stdout(
    ...
    let reason = loop {
        ...
        let read = reader
            ...
            .read_until(b'\n', &mut line);
        match read {
            Ok(0) => break "the pty driver exited",
            Ok(_) if line.len() > MAX_FRAME_BYTES => {
                break "the pty driver emitted an oversized frame";
            }
            Ok(_) => deliver_frame(&proxy, &line),
            Err(_) => break "reading the pty driver's output failed",
        }
    };
...
define_class!(
    #[unsafe(super = NSObject)]
    #[name = "HarnessPtyBridge"]
    struct PtyBridge;

    unsafe impl NSObjectProtocol for PtyBridge {}

    impl PtyBridge {
        #[unsafe(method(deliverFrame:))]
        fn deliver_frame(&self, frame: &NSData) {
            if frame.len() > MAX_FRAME_BYTES {
                bridge_die("the pty service delivered an oversized frame");
            }
            let bytes = frame.to_vec();
...
pub fn bridge() -> std::process::ExitCode {
    autoreleasepool(|_| {
        ...
        let exported = PtyBridge::new();
        unsafe { connection.setExportedObject(Some(&exported)) };

        let interrupted = RcBlock::new(|| bridge_die("the pty service was interrupted"));
        connection.setInterruptionHandler(Some(&interrupted));
        let invalidated = RcBlock::new(|| bridge_die("the pty service connection was invalidated"));
