//! Foundation NSXPC transport for the desktop arch host.
//!
//! ```text
...
const SERVICE: &str = "com.qoder.harness-studio.arch";
const DRIVER_BIN: &str = "harness-arch-host";
const REAP_GRACE: Duration = Duration::from_secs(5);

unsafe extern "C" {
    fn harness_arch_host_protocol() -> *const AnyProtocol;
    fn harness_arch_client_protocol() -> *const AnyProtocol;
}

fn host_interface() -> Retained<NSXPCInterface> {
    unsafe { NSXPCInterface::interfaceWithProtocol(&*harness_arch_host_protocol()) }
}

fn client_interface() -> Retained<NSXPCInterface> {
    unsafe { NSXPCInterface::interfaceWithProtocol(&*harness_arch_client_protocol()) }
}
...
define_class!(
    ...
    #[name = "HarnessArchSession"]
    #[ivars = SharedDriver]
    struct ArchSession;

    unsafe impl NSObjectProtocol for ArchSession {}
...
define_class!(
    #[unsafe(super = NSObject)]
    #[name = "HarnessArchListenerDelegate"]
    ...
    unsafe impl NSXPCListenerDelegate for ListenerDelegate {
        ...
        fn accept(&self, _listener: &NSXPCListener, connection: &NSXPCConnection) -> bool {
            match accept_connection(connection) {
                Ok(()) => true,
                Err(error) => {
                    eprintln!("[arch-host] refused a connection: {error}");
                    false
                }
            }
        }
...
fn accept_connection(connection: &NSXPCConnection) -> io::Result<()> {
    let driver_path = std::env::current_exe()?
        ...
        .ok_or_else(|| io::Error::other("cannot locate the arch host driver"))?;
    ...
    let session = ArchSession::new(shared.clone());
...
fn pump_driver_stdout(
    ...
    let reason = loop {
        ...
        let read = reader
            ...
            .read_until(b'\n', &mut line);
        match read {
            Ok(0) => break "the arch driver exited",
            Ok(_) if line.len() > MAX_FRAME_BYTES => {
                break "the arch driver emitted an oversized frame";
            }
            Ok(_) => deliver_frame(&proxy, &line),
            Err(_) => break "reading the arch driver's output failed",
        }
    };
...
define_class!(
    #[unsafe(super = NSObject)]
    #[name = "HarnessArchBridge"]
    struct ArchBridge;

    unsafe impl NSObjectProtocol for ArchBridge {}

    impl ArchBridge {
        #[unsafe(method(deliverFrame:))]
        fn deliver_frame(&self, frame: &NSData) {
            if frame.len() > MAX_FRAME_BYTES {
                bridge_die("the arch service delivered an oversized frame");
            }
            let bytes = frame.to_vec();
...
pub fn bridge() -> std::process::ExitCode {
    autoreleasepool(|_| {
        ...
        let exported = ArchBridge::new();
        unsafe { connection.setExportedObject(Some(&exported)) };

        let interrupted = RcBlock::new(|| bridge_die("the arch service was interrupted"));
        connection.setInterruptionHandler(Some(&interrupted));
        let invalidated =
            RcBlock::new(|| bridge_die("the arch service connection was invalidated"));
