...
// The global allocator belongs to the final binary that links this crate
// (`harness-diff-host`), not to the library.
...
// The stable surface this crate exposes to `harness-diff-service`. Everything
// else stays internal to the vendored engine.
pub use crate::bridge::{
    structural_diff, StructuralDiff, StructuralLine, StructuralSegment, StructuralSide,
};
...
/// The upstream CLI entrypoint. This crate is built as a library, so the binary
/// pieces are compiled out; `harness-diff-host` owns the process instead.
#[cfg(any())]
fn main() {
