...
/// The same projection `--display json` prints, as a value.
///
/// `crate::bridge` is the only consumer. The engine's internals are
/// `pub(crate)`, so this is the seam that lets the structural-diff bridge read
/// the alignment and change spans instead of recomputing the hunk logic.
pub(crate) fn file_value(diff: &DiffResult) -> serde_json::Value {
    serde_json::to_value(File::from(diff)).expect("a DiffResult always serializes")
}
