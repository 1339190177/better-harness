...
export function connectStudioService(child, { token, dataDirectory, oxcExecutable, oxcTransport = 'stdio', esbuildExecutable, esbuildTransport = 'stdio', acpHostExecutable, acpHostTransport = 'stdio', evidenceHostExecutable, evidenceHostTransport = 'stdio', diffHostExecutable, diffHostTransport = 'stdio', archHostExecutable, archHostTransport = 'stdio', ptyHostExecutable, ptyHostTransport = 'stdio', boxExecExecutable, appsHostUrl, pickDirectory, onFailure, startupTimeout = 30_000, shutdownTimeout = 5_000 }) {
  let ready = false;
  let stopping = false;
  let exited = false;
  let picking = false;
  ...
  child.postMessage(message('start', { token, dataDirectory, oxcExecutable, oxcTransport, esbuildExecutable, esbuildTransport, acpHostExecutable, acpHostTransport, evidenceHostExecutable, evidenceHostTransport, diffHostExecutable, diffHostTransport, archHostExecutable, archHostTransport, ptyHostExecutable, ptyHostTransport, boxExecExecutable, appsHostUrl }));
  return {
    started,
    stop() {
